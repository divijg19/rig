...
We believe in the power of the official Go toolchain. `rig` respects the Go ecosystem, `rig` is not here to replace `go build`, `go mod`, or `go test`. It is here to orchestrate them. It is a no-nonsense tool designed to manage the "meta-work" so you can focus on writing Go.
