...
`rig` ships as one binary (`rig`).

Aliases are optional and work by invocation name (`argv[0]`).

- On macOS/Linux, the shell installer creates aliases as symlinks next to `rig`.
- On Windows, invoke `rig run`, `rig check`, and other commands directly until a PowerShell installer is available.
...
The installer downloads the `rig` binary for your platform and places it on your `PATH`.
...
Additional invocation-name entrypoints are supported and can be created manually if needed:
...
`go install` does not create alias symlinks. If you want aliases, create them manually.

## Installer vs Go Install
