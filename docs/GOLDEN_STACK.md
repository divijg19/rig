**The Golden Stack: Go + Templ + HTMX + Tailwind + Dart & Jaspr / Flutter**
...
- Dart & Jaspr: frontend framework for building web UIs.
...
# Templ (assumes `templ` is installed into .rig/bin via [tools] + `rig sync`)
...
- Use `rig x <tool>` to run a tool that is already pinned in `[tools]` and installed into `.rig/bin`. Example: `rig x golangci-lint -- run ./...`.
